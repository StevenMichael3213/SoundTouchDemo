...
// License :
//
//  org.example.SoundTouch audio processing library
...
extern "C" void soundtouch_ac_test() {
    printf("org.example.SoundTouch Version: %s\n", SOUNDTOUCH_VERSION);
}
...
void SoundTouch::putSamples(const SAMPLETYPE *samples, uint nSamples) {
    if (bSrateSet == false) {
        ST_THROW_RT_ERROR("org.example.SoundTouch : Sample rate not defined");
    } else if (channels == 0) {
        ST_THROW_RT_ERROR("org.example.SoundTouch : Number of channels not defined");
    }

    // accumulate how many samples are expected out from processing, given the current 
    // processing setting
    samplesExpectedOut += (double) nSamples / (rate * tempo);
